        ...
        try {
            ...
            synchronized (mLock) {
                if (mRichTapService != null) {
                    ...
                    success = mRichTapService.richTapVibratorSetAmplitude(strength);
                }
                if (!success && mVibratorInfo.hasCapability(IVibrator.CAP_AMPLITUDE_CONTROL)) {
    ...
    public long on(long vibrationId, long stepId, long milliseconds) {
        ...
        try {
            synchronized (mLock) {
                long duration = 0;
                if (mRichTapService != null) {
                    if (mRichTapService.richTapVibratorOn(milliseconds)) {
    ...
    public long on(long vibrationId, long stepId, PrebakedSegment prebaked) {
        ...
        try {
            synchronized (mLock) {
                ...
                if (pattern != null) {
                    if (mRichTapService.richTapVibratorOnRawPattern(pattern,
