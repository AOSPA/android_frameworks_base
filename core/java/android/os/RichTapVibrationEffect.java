...
public final class RichTapVibrationEffect {
    ...
    public static int[] getPrebakedHeEffect(int id, int strength) {
        ...
        if (!resources.getBoolean(R.bool.config_usesRichtapPrebakedHeResources)) {
            return null;
        }

        String root = resources.getString(R.string.config_richtapPrebakedHeResourceRoot);
        ...
        int[] pattern = loadPrebakedHeEffect(root, strengthDir, fileName);
        if (pattern != null) {
            synchronized (sPrebakedHeCacheLock) {
